...
class CurrencyConversion:
    ...
    def convert_currency(self, value, from_curr, to_curr, exp_date):
        if exp_date and exp_date != str(date.today()):
            yesterday = (datetime.strptime(exp_date, '%Y-%m-%d').date()-timedelta(days=1)).strftime('%Y-%m-%d') # calculating yesterday to use as to date
...
class Group():
    ...
    def total_person(self, person):
        e_person = [exp for exp in self.expenses if exp.payer == person or person in exp.owers]
        e_person_amount = []
...
def exp_for_one_in_cat(group_list, person):
    ...
    for exp in exp_for_person:
        if exp.category in all_cat.keys():
...
def visualize_graph(graph, central_node):
    ...
    for payer, owers in graph.items():
        for ower, amount in owers.items():
            vis_graph.add_edge(payer, ower, weight = float('{:.2f}'.format(amount)))
    ...
    legend_elements = [plt.Line2D([1], [1], color='green', linewidth=4, label='Creditor'), plt.Line2D([1], [1], color='red', linewidth=4, label='Debtor'),
                       plt.Line2D([1], [1], color='yellow', linewidth=4, label='Avizoon'), plt.Line2D([1], [1], color='purple', linewidth=4, label='Mother of Expenses'),
                       plt.Line2D([1], [1], color='blue', linewidth=4, label='No Need to Pay')]
    ax.legend(handles=legend_elements, loc='upper right', title="Legend",fontsize=6, title_fontsize=8)
...
def visualize_pie_chart(total_debts):
    ...
    ax.set_title('Portion of everyone in Unpaid debts',fontsize=10)
    return fig
...
def visualize_bar_chart(group_list, person):
    ...
    ax.set_title("Expenses Shares Bar Chart", fontsize=12)
    ax.set_ylabel("Amount", fontsize=10)
    ax.set_xlabel("Expenses", fontsize=10)
    ax.set_xticks(bar_pos)
    ax.set_xticklabels([f"Exp {exp.name}" for exp in exp_for_person],rotation = 90)
...
def visualize_expenses_for_person(group_list, person):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.pie(values, labels= labels, colors= colors, autopct="%1.1f%%", startangle=90)
    ax.set_title(f"expenses of {person} in each category (total value of each category)", fontsize=10)
    return fig
...
def visualize_last_week(gr_list, person):
    ...
    ax.set_title("Expense trends in last week", fontsize = 14)
...
#pdf_name = "accc_visual_reports.pdf"
'''with PdfPages(pdf_name) as pdf:
    # second page of pdf report
