...
def CreateCalss(file_path, out_curr):
    ...
    for _, row in data.iterrows():
        ...
        owers = [people for i, share in enumerate(shares_list) if share > 0]
        ...
        expense = Expense(name= name, value= value, payer= payer, owers= owers, group_name= group_name, category= category,
                          ex_date= exp_date, split_type= 'exact', shares= shares_list, recurring= recurrent_mode,
...
def visualize_bar_chart(group_list, person):
    ...
    all_people = set(person for exp in exp_for_person for person in exp.shares_dict.keys())
